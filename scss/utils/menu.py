...
from .string import format_repr
...
def get_menu_item(menu_id, item, **kwargs):
    ...
    """
    return None #named_objects_from_signal(signals.menu.items.send(menu_id, **kwargs)).get(item)
...
def render_sidemenu(menu_id, active_item=None, **kwargs):
    ...
    return render_template('side_menu.html', items=items, menu_id=menu_id)