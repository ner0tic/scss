...
def registered_users():
    ...
    return render_template(
        'admin/registered_users.jinja2', users=users, roles=roles)
...
def user_info(user_id):
    ...
    if user is None:
        abort(404)
    return render_template('admin/manage_user.jinja2', user=user)
...
def change_account_info(user_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Account information successfully updated to {format(user.full_name(), user.role.name)}.', 'form-success')
    return render_template('admin/manage_user.jinja2', user=user, form=form)
