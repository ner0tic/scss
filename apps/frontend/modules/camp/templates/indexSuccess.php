...
<?php foreach($pager->getResults() as $i => $camp): ?>
        <tr class="table-row <?php echo fmod($i,2) ? 'even-row' : 'odd-row' ?>">
          <td><?php echo link_to(ucwords($camp->getName()), @weeks_by_camp,array(
                          'country_slug'  =>  $camp->getDistrict()->getZone()->getCountry()->getSlug(),
                          'zone_slug'     =>  $camp->getDistrict()->getZone()->getSlug(),
                          'district_slug' =>  $camp->getDistrict()->getSlug(),
                          'camp_slug'     =>  $camp->getSlug())) ?></td>
<!--          <td></td> -->
          <td><?php echo $camp->getDistrict()->getName().', '.$camp->getDistrict()->getZone()->getCode() ?></td>
          <td class="table-controls">
            <div class="table-controls">
              <a href="<?php echo url_for('camp/edit?id='.$camp->getId()) ?>">
              <a href="<?php echo url_for('camp/edit?id='.$camp->getId()) ?>">
            ...
            <tr>
              <td colspan="3">
                <div class="pagination_desc">
                  <strong><?php echo count($pager) ?></strong> camps found
